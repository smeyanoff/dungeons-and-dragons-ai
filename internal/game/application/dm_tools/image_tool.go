...
func (t *GenerateImageTool) Description() string {
	return `Генерирует изображение через AI по текстовому описанию. Используй ТОЛЬКО при явном запросе игрока ("нарисуй", "покажи картинку") или при крупном событии: новая локация, ключевой NPC, конец боя. Не генерируй изображения без причины.
...
Примеры: игрок просит "нарисуй древний замок" или произошло значимое событие (новая локация, ключевой NPC). Описание должно быть детальным и атмосферным, в стиле фэнтези и D&D. Изображения кэшируются.`
}
