...
func (t *ValidateItemUsageTool) Description() string {
	return "Проверить наличие предмета в инвентаре персонажа перед его использованием. " +
		"Используй этот инструмент ТОЛЬКО когда игрок пытается ИСПОЛЬЗОВАТЬ предмет из инвентаря (выпить зелье, надеть доспех, применить предмет, открыть замок/дверь ключом, пройти преграду квестовым предметом и т.д.). " +
		"ОБЯЗАТЕЛЬНО вызови этот инструмент, если по сюжету проход/дверь/преграда требует конкретный предмет, ПЕРЕД тем как решить, что игрок прошёл дальше — если предмета нет, путь остаётся закрыт, не выдумывай, что предмет всё равно нашёлся. " +
