	...
	if currentLoc != nil {
		...
		if scenario := currentLoc.GetScenario(); scenario != nil {
			...
			if len(scenario.RelatedNPCNames) > 0 || len(scenario.RelatedQuestItemNames) > 0 {
				...
				if len(scenario.RelatedQuestItemNames) > 0 {
					parts = append(parts, "⚠️ Если проход дальше или ключевое событие сцены по сюжету требует один из квестовых предметов выше — сначала проверь инвентарь партии инструментом 'get_inventory' или 'validate_item_usage'. Нет предмета — не открывай путь и не продвигай сюжет, опиши это как препятствие, а не выдумывай, что предмет нашёлся.")
				}
			}
